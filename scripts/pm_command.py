...
def main() -> int:
    ...
    try:
        ...
        r = rewrite(raw, target, retries=1)
        print(json.dumps({
        ...
        with log_path.open("a", encoding="utf-8") as f:
            f.write(f"{__import__('time').strftime('%Y-%m-%d %H:%M:%S')} ok target={target} raw={raw[:60]!r}\n")
    except Exception as e:
        print(json.dumps({"error": f"{type(e).__name__}: {e}"}, ensure_ascii=False))
        with log_path.open("a", encoding="utf-8") as f:
            f.write(f"{__import__('time').strftime('%Y-%m-%d %H:%M:%S')} ERROR {type(e).__name__}: {e}\n")
    return 0
